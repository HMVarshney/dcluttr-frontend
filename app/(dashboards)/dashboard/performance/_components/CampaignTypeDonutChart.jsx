    ...
    if (active && payload && payload.length) {
      return (
        <div className=" border rounded shadow flex flex-col bg-white px-4 py-2">
          <div className='text-xs font-semibold text-[#0A090B4D]'>
            {payload[0].name}
          </div>
          <div className='text-base font-bold text-[#0A090B]'>
            {payload[0].value}
          </div>
        </div>
      );
      ...
      <div className='border border-[#F1F1F1] shadow-[0px_1px_0px_0px_rgba(0,0,0,0.12)] rounded-lg bg-white w-full h-[calc(100%-44px)] overflow-hidden group flex flex-col justify-between items-stretch'>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
              ...
              dataKey="value"
            >
              {selected?.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
            </Pie>
        ...
        <div className='py-4 px-5 text-xs font-semibold border-t border-[#F1F1F1] flex items-center gap-4 overflow-x-auto'>
          {selected?.map((entry, index) => (
            <div key={entry.campaign_type} className='py-2.5 px-4 rounded-full border flex items-center gap-2 bg-white cursor-pointer'
              onClick={() => setSelected(selected.map(item => ({ ...item, active: item.campaign_type === entry.campaign_type ? !item.active : item.active })))} >
