...
function GoogleConnectModal({ openModal, onClose, value, setValue, handleSubmit }) {
  return (
...
function ConnectGoogle({ openModal, onClose, brandId, orgId }) {
  ...
  const { googleConnectLoading } = useSelector((state) => state.sourceConnect);

  const handleSubmit = () => {
    if (!value) return;
    dispatch(connectGoogle({ brandId, orgId, value }))
      .unwrap()
      .then(() => {
