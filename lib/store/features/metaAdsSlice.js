...
export const getCreativeMeta = createAsyncThunk("creative/getCreativeMeta", async (_, { dispatch }) => {
  try {
    const api = cubeJsApi();
    const res = await api.load({
      measures: [
...
const metaAdsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // getCreativeMeta
      .addCase(getCreativeMeta.pending, (state) => {
        state.creativeLoading = true;
        state.creativeError = null;
      })
      .addCase(getCreativeMeta.fulfilled, (state, action) => {
      ...
      .addCase(getCreativeMeta.rejected, (state, action) => {
        state.creativeLoading = false;
